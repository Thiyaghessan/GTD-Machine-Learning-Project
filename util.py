#Helper functions and classes for ML project
import pandas as pd
import datetime
import numpy as np
...
def duration_calculator(row):
    """
    Calculates the duration passed between two dates
    """
    if row["extended"] == 1:
        if row["imonth"] == 0 or row["iday"] == 0:
            return 0
        else:
            start_date = datetime.datetime(row["iyear"], row["imonth"], row["iday"])
            end_date = row["resolution"]
            duration = abs((end_date - start_date).days)
            return duration
    else:
        return 0
    
def text_counter(row, feature):
    """
    Calculates the number of entries in a cell with format "entry1; entry2; entry3"
    """
    if type(row[feature]) == str:
        return len(row[feature].split(","))
    else:
        return 0

def num_to_string_encoder(row, feature):
    """
    Transforms numerical categories to string categories
    """
    if row[feature] == 1:
        return "Yes"
    elif row[feature] == 0:
...
def con_matrix(test, preds, name, target_names):
    """
    Builds a confusion matrix when given predicted y-values, actual y-values and a name for the model
    """
    matrix = confusion_matrix(test, preds)
    ...
    title = "Confusion matrix for " + name + " model"
    plt.title(title)
    plt.show()
...
class FeatureImportance:
    ...
    def get_feature_names(self, verbose=None):  
        ...
        Note: 
        If the ColumnTransformer contains Pipelines and if one of the transformers in the Pipeline is adding completely new             columns, it must come last in the pipeline. For example, OneHotEncoder, MissingIndicator &                                       SimpleImputer(add_indicator=True) add columns to the dataset that didn't exist before, so there should come last in the         Pipeline.
...
#             title_text = f'Top {top_n_features} (of {n_all_importances}) Feature Importances'       
            title_text = f'Top {top_n_features} Feature Importances'       
